...
const clickupFrontmatter = {
  ...
  title: task.name,
  status: mapStatusFromClickUp(task.custom_fields.find(f => f.name === 's
...
**Dependencies:**
- `common/utils/story-manager.js` - saveStoryFile, parseStoryFile
- `common/utils/status-mapper.js` - mapStatusFromClickUp
